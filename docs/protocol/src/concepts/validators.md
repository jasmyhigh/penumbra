...
* **Inactive**: a validator whose delegation pool is too small to participate in the consensus set
...
1. they can become active again, if new delegations boost their weight back into the top N;
...
If (2) occurs, the same state transitions as in regular tombstoning occur: all pending undelegations are canceled, etc.
